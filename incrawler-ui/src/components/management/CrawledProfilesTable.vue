  ...
  <div>
    ...
    >
      <template #item.username="{ item }">
        <a :href="`http://www.linkedin.com/in/${item.referenceKey}`">
          {{ item.username }}
        </a>
      </template>
      <template v-slot:top>
        <v-toolbar
            flat
        >
          <v-toolbar-title>Crawled Profiles</v-toolbar-title>
          <v-divider
              class="mx-4"
              inset
              vertical
          ...
          <v-dialog v-model="dialogProcess" max-width="600px">
            <v-card>
              <v-card-title class="headline">Are you sure you want to process this profile?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      {text: 'Is Processed', value: 'isProcessed', sortable: true},
      {text: 'Last Process Date', value: 'lastProcessDate', sortable: true},
      {text: 'Actions', value: 'actions', sortable: false},
    ],
    crawledProfiles: [],
    selectedCrawledProfile: {
      username: "",
      lastCrawlDate: null,
      isProcessed: "",
  ...
  methods: {
    ...
    initialize() {
      ...
      if (sortBy.length > 0) {
        ...
        } else if (sortText == "isProcessed") {//bu alan son process tarihine gore dolduruldugu icin bu alana gore siralanacak
          sortText = "lastProcessDate";
        }
      }
      axiosInstance.get(`get-all-crawled-profiles?` + "&search=" + this.search + "&page=" + page + "&size=" + itemsPerPage + "&sortBy=" + sortText + "&sortDesc=" + sortDesc)
          .then(response => {
            if (response.status == 200) {
              this.crawledProfiles = [];
              ...
              response.data.items.forEach(function (item, index) {
                const resItem = {
                  ...
                  isProcessed: item.lastProcessDate != null ? "Processed" : "Not Processed"
                }
                this.crawledProfiles.push(resItem)
              }, this);
            }
          })
          .catch(e => {
    ...
    recrawlProfile(item) {
      this.selectedCrawledProfile = Object.assign({}, item)
    ...
    deleteItemConfirm() {
      this.showPopup("Crawled profile will be deleted...");
      axiosInstance.delete("delete-crawled-profile/" + this.selectedCrawledProfile.referenceKey)
          .then(response => {
            if (response.status == 200) {

              const index = this.crawledProfiles.findIndex(value => value.referenceKey == response.data);
              Vue.$log.info("ref : " + response.data + " index : " + index)
              if (index > -1) {
                this.crawledProfiles.splice(index, 1);
                this.serverItemsLength -= 1;
                this.showPopup("Crawled profile  deleted");

                Vue.$log.debug("Crawled profile deleted : " + response.data);
              }
            }
            this.loading = false;
          })
          .catch(e => {
            this.showPopup("Could not deleted crawled profile...");
            Vue.$log.error(e);
            this.loading = false;
          })
    ...
    recrawlingConfirm() {
      ...
      const data = {
        referenceKey: this.selectedCrawledProfile.referenceKey
      }
      axiosInstance.post("recrawl-profile/", data)
          .then(response => {
            if (response.status == 200) {
              const index = this.crawledProfiles.findIndex(value => value.referenceKey == response.data.referenceKey);
              Vue.$log.debug("ref : " + response.data + " index : " + index)
              if (index > -1) {
                this.showPopup("Profile recrawling started...");
                Vue.$log.debug("Profile recrawling started : " + response.data);
              }
            }
            this.loading = false;
          })
            ...
            if (response.status == 200) {
              ...
              if (index > -1) {
                const resItem = {
                  ...
                  lastCrawlDate: this.format_date(response.data.lastCrawlDate),
                  lastProcessDate: this.format_date(response.data.lastProcessDate),
                  referenceKey: response.data.referenceKey,
                  isProcessed: response.data.lastProcessDate != null ? "Processed" : "Not Processed"
                }
                Object.assign(this.crawledProfiles[index], resItem)
                this.showPopup("Profile processing completed");

                Vue.$log.debug("Profile processing completed : " + response.data);
              }
            }
            this.loading = false;
          })
    ...
    close() {
      this.dialog = false
      this.$nextTick(() => {
        this.selectedCrawledProfile = Object.assign({}, this.defaultItem)
      })
